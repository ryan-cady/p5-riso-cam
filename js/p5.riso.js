...
function ditherImage(img, type, threshold) {
  ...
  // Modified Floyd-Steinberg that responds to threshold like other algorithms
      let currentPixel = Math.max(0, Math.min(255, out.pixels[i]));
      newPixel = currentPixel < threshold ? 0 : 255;
      
      // Reduce error diffusion to make threshold more influential
      err = (currentPixel - newPixel) * 0.75 / 16; // Scale down error by 0.75
      out.pixels[i] = newPixel;
      
      if (i + 4 < out.pixels.length) out.pixels[i + 4] += err * 7;
      if (i + 4 * w - 4 >= 0 && i + 4 * w - 4 < out.pixels.length) out.pixels[i + 4 * w - 4] += err * 3;
      if (i + 4 * w < out.pixels.length) out.pixels[i + 4 * w] += err * 5;
      if (i + 4 * w + 4 < out.pixels.length) out.pixels[i + 4 * w + 4] += err * 1;
        
    } else {
      // Bill Atkinson's dithering algorithm
      newPixel = out.pixels[i] < threshold ? 0 : 255;
